 ...
 */
void do_pwm_on(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void do_pwm_off(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  analogWrite(pin_ctrl_ptr->pin, 0);
  pin_ctrl_ptr->hold_value = 0;
  DBG_PRINTLN(String(F("#PWM: Off : ")) + pin_ctrl_ptr->name);
}

void do_pwm_toggle(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void do_pwm_set_duty(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr, int percent_duty) {
...
void print_status_pwm(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  sys->serial->print("0x");
  sys->serial->println(pin_ctrl_ptr->tunable.pwm.duty, HEX);
}

bool type_change_to_pwm(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
...
bool type_change_from_pwm(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  do_pwm_off(sys, pin_ctrl_ptr);
...
void factory_reset_tunables_pwm(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  pin_ctrl_ptr->tunable.duration = DEFAULT_SW_PUSH_DURATION;
  pin_ctrl_ptr->tunable.pwm.duty = 0;
}

void setup_pwm(struct sysvar_per_core_t * sys) {
  for (int i = 0; i < NR_PIN_CTRL_ENTRY; i++) {
    if (PIN_TYPE_PWM == pin_ctrl_tbl[i].tunable.type) {
      ...
      type_change_to_pwm(sys, &pin_ctrl_tbl[i]);
    }
  }
}
