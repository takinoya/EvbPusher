 ...
 */
void do_sw_on(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void do_sw_off(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  if (PIN_POL_ACTIVE_LO == pin_ctrl_ptr->tunable.sw.polarity) {
    digitalWrite(pin_ctrl_ptr->pin, HIGH);
  } else {
    digitalWrite(pin_ctrl_ptr->pin, LOW);
...
void do_sw_toggle(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  int toggled;
 ...
 */
void print_status_sw(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  int pin_stat = digitalRead(pin_ctrl_ptr->pin);
  if (PIN_POL_ACTIVE_LO == pin_ctrl_ptr->tunable.sw.polarity) {
    sys->serial->println( (pin_stat==HIGH)? F("OFF,1") : F("ON,0") );
  }
  else {
    sys->serial->println( (pin_stat==HIGH)? F("ON,1") : F("OFF,0") );
  }
}
 ...
 */
bool type_change_to_sw(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  for (int i=0; i < sizeof(pin_black_list_digital)/sizeof(pin_black_list_digital[0]); i++) {
    if (pin_black_list_digital[i] == pin_ctrl_ptr->pin) {
      // In black list : NG
      sys->last_cmd_error_mesg = F("The pin is in Black-list !?!?");
      return false;
    }
  }
...
bool type_change_from_sw(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
...
void factory_reset_tunables_sw(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  pin_ctrl_ptr->tunable.duration = DEFAULT_SW_PUSH_DURATION;
  pin_ctrl_ptr->tunable.sw.polarity = PIN_POL_ACTIVE_HI;
}

void setup_sw(struct sysvar_per_core_t * sys) {
  for (int i = 0; i < NR_PIN_CTRL_ENTRY; i++) {
    if (PIN_TYPE_SW == pin_ctrl_tbl[i].tunable.type) {
      ...
      type_change_to_sw(sys, &pin_ctrl_tbl[i]);
      do_sw_off(sys, &pin_ctrl_tbl[i]);
    }
  }
}
 ...
 */
void print_status_in(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  int pin_stat = digitalRead(pin_ctrl_ptr->pin);
  if (PIN_POL_ACTIVE_LO == pin_ctrl_ptr->tunable.sw.polarity) {
    sys->serial->println( (pin_stat==HIGH)? F("OFF,1") : F("ON,0") );
  }
 ...
 */
bool type_change_to_in(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  for (int i=0; i < sizeof(pin_black_list_digital)/sizeof(pin_black_list_digital[0]); i++) {
    if (pin_black_list_digital[i] == pin_ctrl_ptr->pin) {
      // In black list : NG
...
bool type_change_from_in(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  pin_ctrl_ptr->tunable.type = PIN_TYPE_VOID;
...
void factory_reset_tunables_in(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  pin_ctrl_ptr->tunable.duration = DEFAULT_SW_PUSH_DURATION;
  pin_ctrl_ptr->tunable.sw.polarity = PIN_POL_ACTIVE_HI;
}

void setup_in(struct sysvar_per_core_t * sys) {
  for (int i = 0; i < NR_PIN_CTRL_ENTRY; i++) {
    if (PIN_TYPE_IN == pin_ctrl_tbl[i].tunable.type) {
      ...
      INIT_PRINT(F(", Pin = ") );
      INIT_PRINTLN(pin_ctrl_tbl[i].pin, DEC);
      type_change_to_in(sys, &pin_ctrl_tbl[i]);
    }
  }
}
