 ...
 *
 * AVR EEPROM usage : Based on NanoEvery 256Byte
 * +=====================+==============+=====================================+
 ...
 * +=====================+==============+=====================================+
 *
 * (TBD) Nano R4 (Cortex-M) EEPROM usage : 512 of 4096 Byte
 * +=====================+==============+=====================================+
 * | Contents            | Offset range | Note                                |
 * +=====================+==============+=====================================+
 * | header.crc          | +0..3        | unisgned long                       |
 ...
 * | debug_flags         | +7           | uint8_t                             |
 * +---------------------+--------------+-------------------------------------+
 * | tunable_tbl[0]      | +8..         | 8byte                               |
 * +---------------------+--------------+-------------------------------------+
 * | tunable_tbl[1]      | +16..        |                                     |
 * +---------------------+--------------+-------------------------------------+
 * |     ....            |              |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | tunable_tbl[16]     | +136..143    |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | pin_name_str_tbl[0] | +144..151    | Fixed 8byte strings                 |
 * +---------------------+--------------+-------------------------------------+
 * | pin_name_str_tbl[1] | +152..159    |                                     |
 * +---------------------+--------------+-------------------------------------+
 * |     ....            |              |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | pin_name_str_tbl[16]| +272..279    |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | (Reserved)          | ...512       | Remaining area                      |
 * +=====================+==============+=====================================+
 *
 * Xiao RP2040 Emulated EEPROM usage : 256Byte
 * +=====================+==============+=====================================+
 * | Contents            | Offset range | Note                                |
 * +=====================+==============+=====================================+
 * | header.crc          | +0..3        | unisgned long                       |
 ...
 * | tunable_tbl[8]     | +72..79       |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | pin_name_str_tbl[0] | +80..87      | Fixed 8byte strings                 |
 * +---------------------+--------------+-------------------------------------+
 * | pin_name_str_tbl[1] | +88..95      |                                     |
 * +---------------------+--------------+-------------------------------------+
 * |     ....            |              |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | pin_name_str_tbl[8] | +144..151    |                                     |
 * +---------------------+--------------+-------------------------------------+
 * | (Reserved)          | ...255       | Remaining area                      |
 * +=====================+==============+=====================================+
 *
 * (TBD) RaspberryPi Pico Emulated EEPROM usage : 512Byte
 * +=====================+==============+=====================================+
 * | Contents            | Offset range | Note                                |
 * +=====================+==============+=====================================+
 * | header.crc          | +0..3        | unisgned long                       |
...
int save_to_eeprom(struct sysvar_per_core_t * sys, bool with_debug_flag)
{
  // Save padding6
 ...
 */
int load_from_eeprom(struct sysvar_per_core_t * sys, bool with_debug_flag)
{
  ...
  if (IS_INIT_DEBUG_ENABLED) {
    sys->serial->println(F("Load: pin_ctrl_tbl before loaded from EEPROM:"));
    cmd_list(sys);
  }
  // Check header
  int eeAddr = 0;
  ...
  if (crc != saved_header.crc) {
    sys->serial->println(F("#Warning! EEPROM : CRC not matched --> Skip loading.") );
    return 0;
  }
  if (SAVE_DATA_FORMAT_VERSION_MAJOR != saved_header.major) {
    sys->serial->println(F("#Warning! EEPROM : Save-Format version major not matched --> Skip loadig.") );
    return 0;
  }
  if (SAVE_DATA_FORMAT_VERSION_MINOR != saved_header.minor) {
    sys->serial->println(F("#Warning! EEPROM : Save-Format version minor not matched --> Skip loadig.") );
    return 0;
  }
  ...
  if (IS_INIT_DEBUG_ENABLED) {
    sys->serial->println(F("Load: pin_ctrl_tbl is loaded from EEPROM:"));
    cmd_list(sys);
  }
  return 0;
}
 ...
 */
void fill_eeprom_data(struct sysvar_per_core_t * sys, uint8_t data) {
  for (int i = 0 ; i < EEPROM_SAVE_REGION_LEN ; i++) {
    EEPROM.update(i, data);
  }
}

void setup_eeprom(struct sysvar_per_core_t * sys) {
#if defined(ARDUINO_SEEED_XIAO_RP2040) || defined(ARDUINO_RASPBERRY_PI_PICO)
  EEPROM.begin(EEPROM_SAVE_REGION_LEN);
#endif
}
