...
void do_servo_down(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void do_servo_up(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void do_servo_n(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void do_servo_toggle(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
...
void print_status_servo(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr) {
  sys->serial->print(pin_ctrl_ptr->srv_obj->read(), DEC);
  sys->serial->print(F(",dn="));
  sys->serial->print(pin_ctrl_ptr->tunable.srv.deg_dn, DEC);
  sys->serial->print(F(",n="));
  sys->serial->print(pin_ctrl_ptr->tunable.srv.deg_n, DEC);
  sys->serial->print(F(",up="));
  sys->serial->print(pin_ctrl_ptr->tunable.srv.deg_up, DEC);
  sys->serial->print(F(",duration="));
  sys->serial->println(pin_ctrl_ptr->tunable.duration, DEC);
}

Servo* search_vacant_servo_instance(struct sysvar_per_core_t * sys)
{
  // Confirm vacancy
...
bool type_change_to_srv(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
  for (int i=0; i < sizeof(pin_black_list_digital)/sizeof(pin_black_list_digital[0]); i++) {
    if (pin_black_list_digital[i] == pin_ctrl_ptr->pin) {
      // In black list : NG
      sys->last_cmd_error_mesg = F("Black listed pin !?!?");
      return false;
    }
  }
  Servo* vacant_srv_obj = search_vacant_servo_instance(sys);
  if (nullptr == vacant_srv_obj) {
    // No vacant instance : NG
    sys->last_cmd_error_mesg = F("No vacant Servo instance !?!?");
    return false;
  }
...
bool type_change_from_srv(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
...
void factory_reset_tunables_servo(struct sysvar_per_core_t * sys, struct pin_ctrl_entry_t* pin_ctrl_ptr)
{
...
void setup_servo(struct sysvar_per_core_t * sys) {
  for (int i = 0; i < NR_PIN_CTRL_ENTRY; i++) {
    if (PIN_TYPE_SRV == pin_ctrl_tbl[i].tunable.type) {
      ...
      type_change_to_srv(sys, &pin_ctrl_tbl[i]);
    }
  }
}
